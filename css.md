...
`vw/vh`: a CSS unit that represents the percentage relatively to the container
...
`%`: a CSS unit that represents the view height. We use it usually for background size and set it to 100 to get the size of the whole screen vertically.
...
/*To make any item in the container doesn't care about the container content, and it doesn't mind being on top of the over elements we use position: fixed
This is perfect to put things stacking on top of each other
Making the position fixed allows you to use the following properties
- top:
- bottom: 
- left: 
- right: 
...
/*When position is (absolute), you can put things stacking on each other, but you can specify which element should be on visible on the top of another element, we use top and add a higher number to it */
top: 1000;
```
